...
bool Bigger(TreeNode* a, TreeNode* b)
{
...
vector<int> max_depth(TreeNode* a)
{
	vector<int> ret;
...
bool compatible(TreeNode* a, TreeNode* b)
{
	return Bigger(a, b) || Bigger(b, a);
}

// insert B to A
void InsertNode(TreeNode* a_parent, TreeNode* a, TreeNode* b)
{
	if (Bigger(b, a))
	{
		for (auto it = a->children.begin(); it != a->children.end(); it++)
