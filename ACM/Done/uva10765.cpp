...
void get_dfs_info(int node_count, vector<vector<int>>& edges, vector<int>& dfs_index, vector<int>& dfs_min_reachable)
{
...
void get_connected_component_count2(int node_count, vector<vector<int>>& edges, vector<bool>& visited, int index, int skip_node)
{
	visited[index] = true;
...
int get_connected_component_count(int node_count, vector<vector<int>>& edges, int skip_node)
{
	vector<bool> visited(node_count, false);
	int count = 0;
...
bool sort_function(pair<int, int> p1, pair<int, int> p2)
{
